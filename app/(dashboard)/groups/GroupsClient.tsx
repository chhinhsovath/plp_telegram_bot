...
import { Users, MessageSquare, Calendar, MoreVertical, Filter } from "lucide-react";
      ...
      {groups.length === 0 ? (
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">No groups match filter</h3>
            <p className="text-gray-600 dark:text-gray-400">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredGroups.map((group) => (
            <Card 
              key={group.id} 
              className={`hover:shadow-lg transition-shadow ${!group.isActive ? 'opacity-75' : ''}`}
            >
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <CardDescription>
                      {group.username ? `@${group.username}` : `ID: ${group.telegramId.toString()}`}
                    </CardDescription>
                  </div>
